...
- `dp(increment, minReps, maxReps)`: each fully completed session adds one rep; at max it adds the increment and resets reps to min. Write the exercise at min reps, e.g. `3x8` for an 8 to 12 range.
- One progression per exercise across the whole program. The same exercise with different equipment is still the same exercise. Use a label to split them: `overhead: Triceps Extension, Cable` and `db: Triceps Extension, Dumbbell`.
...
- Custom progression, used for assisted chin-ups:

```
Chin Up, Leverage Machine / 3x6 40kg 120s / warmup: 1x5 55kg / progress: custom() {~
  if (completedReps >= reps) {
    if (reps[1] < 10) { reps += 1 } else { reps = 6; weights -= 2.5kg }
  }
~}
```
...
Names must match the library exactly. Equipment labels: `Barbell`, `Dumbbell`, `Cable`, `Leverage Machine`, `Smith Machine`, `EZ Bar`, `Bodyweight`, `Band`, `Kettlebell`, `Trap Bar`.
...
| Overhead cable extension | `overhead: Triceps Extension, Cable` |
| Dumbbell overhead extension | `db: Triceps Extension, Dumbbell` |
